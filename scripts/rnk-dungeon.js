...
class RNKSceneTracker extends AppV2Base {
  ...
  static PARTS = {
    form: {
      template: "modules/rnk-alpha-mapgen/templates/scene-tracker.html"
    }
  };
...
class RnkDungeonGmHub extends AppV2Base {
  ...
  static PARTS = {
    form: {
      template: "modules/rnk-alpha-mapgen/templates/gm-hub.html"
    }
  };
    ...
    for (let i = 0; i < results.length - 1; i++) {
        ...
        if (exitPos && entrancePos) {
            ...
            await levelA.scene.createEmbeddedDocuments("Tile", [{
                ...
                alpha: 0.85,
                hidden: false,
                flags: { "rnk-alpha-mapgen": { type: "stairTile", targetScene: levelB.scene.id } }
            }]);
            ...
            await levelB.scene.createEmbeddedDocuments("Tile", [{
                ...
                alpha: 0.85,
                hidden: false,
                flags: { "rnk-alpha-mapgen": { type: "stairTile", targetScene: levelA.scene.id } }
            }]);
        }
    }
  }
  ...
  async _fetchPresets() {
    ...
    try {
      ...
      if (response.status === 404) {
        console.warn("RNK Alpha MapGen: presets endpoint unavailable; using bundled presets.");
        return;
      }
      ...
      console.warn("RNK Alpha MapGen: Failed to fetch presets; using bundled presets.", err);
    }
  }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 401 || response.status === 403) {
          ui.notifications.warn("RNK Alpha MapGen: Patreon access is required for this generation.");
        } else if (response.status === 429) {
          ui.notifications.warn("RNK Alpha MapGen: Daily quota reached. Try again tomorrow or upgrade tiers.");
        }
          ...
          for (const pData of patrolsData) {
            ...
            for (let i = 0; i < pData.waypoints.length; i++) {
              ...
              waypoints[wpId] = {
                ...
                y: wp.y * tileSize,
                sceneId: scene.id,
                flags: { "rnk-alpha-mapgen": { patrolId: pData.id } }
              };
            }
        ...
        for (const prop of result.props) {
          ...
          if (prop.category === "NPC") {
            ...
            tiles.push({
              ...
              alpha: 1,
              hidden: false,
              flags: { "rnk-alpha-mapgen": { propId: prop.id, data: prop, isAmbientNPC: true } }
            });
            ...
            if (img) {
              ...
              if (isValid) {
                tiles.push({
                  ...
                  width: (prop.metadata?.size?.[0] || 1) * tileSize,
                  height: (prop.metadata?.size?.[1] || 1) * tileSize,
                  flags: { "rnk-alpha-mapgen": { propId: prop.id, data: prop } }
                });
              }
            }
          }
  ...
  async _generateSounds(geometry, tileSize, opts = {}) {
    const sounds = [];
    const SOUND_BASE = "modules/rnk-alpha-mapgen/assets/sounds";
...
Hooks.on("renderSceneDirectory", (app, html) => {
  ...
  // Add RNK Alpha MapGen button next to the create button for quick access
  if (createButton.length && !$html.find(".rnk-alpha-mapgen-scene-button").length) {
    const btn = $(`
      <button class="rnk-alpha-mapgen-scene-button rnk-alpha-mapgen-button" type="button" title="RNK Alpha MapGen">
        <i class="fas fa-map"></i> RNK Alpha MapGen
      </button>
    `);
    btn.on("click", (ev) => {
      ev.preventDefault();
...
Hooks.on("getSceneControlButtons", (controls) => {
  try {
    ...
    const gmHubTool = {
      ...
      onChange: (active) => {
        ...
        if (hub) {
          ...
          ui.notifications.error("RNK Dimdraft: GM Hub not initialized.");
        }
      }
    };
    ...
    if (Array.isArray(controls)) {
      ...
      controls["rnk-alpha-mapgen"] = control;
    }

  } catch (err) {
    console.error("RNK Dimdraft: Failed to register scene control group", err);
  }
});

// Bind a direct click listener to the scene-control tab so clicking the
// main RNK Alpha MapGen button in the left sidebar opens the GM Hub immediately
...
Hooks.on("renderSceneControls", (app, html) => {
  try {
    ...
    const btn = root.querySelector('[data-control="rnk-alpha-mapgen"]');
    if (!btn || btn.dataset.rnkHubBound === "1") return;
    btn.dataset.rnkHubBound = "1";
    btn.addEventListener("click", () => {
      const hub = window.RnkDungeonGmHub || game.rnkMapper?.RnkDungeonGmHub;
      if (hub) hub.instance.render({ force: true });
      else ui.notifications.error("RNK Dimdraft: GM Hub not initialized.");
    });
  } catch (err) {
    console.error("RNK Dimdraft: Failed to bind scene-control click", err);
  }
});
